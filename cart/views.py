...
def cart_details(request, tot=0, count=0, ct_items=None):
    try:
        ...
        pass
    return render(request, 'cart_page.html', {'cart_items': ct_items, 'total_price': tot, 'count': count})


def c_id(request):
    ct_id = request.session.session_key
    print("own",request.session.session_key)
    if not ct_id:
        print("new")
        ct_id = request.session.create()
        print("new",request.session.create())
    return ct_id
